...
t_string	*string_trim(t_string *s)
{
	...
	start = 0;
	while (start < s->length && (s->data[start] == ' ' || 
...
int main(void)
{
	...
	t_string *with_spaces = string_new("   trim me   ");
	printf("   Original: \"%s\" (length: %zu)\n", 
		with_spaces->data, with_spaces->length);
	...
	t_string *greeting = string_new("hello, ");
	t_string *name = string_new("garbage collector");
	t_string *combined = string_concat(greeting, name);
	t_string *final = string_toupper(combined);
	printf("   Final: \"%s\"\n\n", final->data);
